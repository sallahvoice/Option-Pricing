...
class OutputRepository(BaseRepository):
    ...
    def get_outputs_by_scenario(self, calculation_id: int, vol_shock: float, stock_shock: float):
        query = f"SELECT * FROM {self.table} WHERE CalculationId = %s AND VolatilityShock = %s AND StockPriceShock = %s"
...
#delete_outputs_by_scenario(calculation_id: int, vol_shock: float, stock_shock: float) (min, max, avg, count of outputs given a calc id)
